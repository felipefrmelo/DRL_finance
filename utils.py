...
def get_env_param():
    ...
    acp_right_col = [('Fixed Cost of Selling per Share:', ['${:.3f}'.format(env.epsilon)]),
                     ...
                     ('Single Step Variance:', [
                      '{:.3f}'.format(env.singleStepVariance)]),
...
#     print('Standard Deviation of Random Noise: {:,.5f}'.format(np.sqrt(env.singleStepVariance * env.tau)))
    ...
    price_df = pd.DataFrame(data=price_hist,  columns=[
                            'Stock'], dtype='float64')
...
def plot_efficient_frontier(tr_risk=1e-6):
    ...
    plt.colorbar(sc, label='AC Utility',
...
def plot_trade_list(lq_time=60, nm_trades=60, tr_risk=1e-6, show_trl=False):
    ...
    df = pd.DataFrame(data=list(range(nm_trades + 1)),
                      columns=['Trade Number'], dtype='float64')
    ...
    axes[0].plot(df['Trade Number'].iloc[1:],
                 df['Stocks Sold'].iloc[1:], linewidth=2.0, alpha=0.5)
    ...
    axes[1].plot(df['Trade Number'], df['Stocks Remaining'],
...
#         rd_trl = new_trl
        ...
        df2['Stocks Remaining'] = (
            np.ones(nm_trades + 1) * env.total_shares) - np.cumsum(rd_trl)
...
def implement_trade_list(seed=0, lq_time=60, nm_trades=60, tr_risk=1e-6):
    ...
    # Reset the environment with the given parameters
    env.reset(seed=seed, liquid_time=lq_time,
              num_trades=nm_trades, lamb=tr_risk)
...
def get_av_std(lq_time=60, nm_trades=60, tr_risk=1e-6, trs=100):
    ...
    for episode in range(trs):
        ...
        # Reset the enviroment
        env.reset(seed=episode, liquid_time=lq_time,
                  num_trades=nm_trades, lamb=tr_risk)
    ...
    print('Average Implementation Shortfall: ${:,.2f}'.format(
        shortfall_hist.mean()))
    print('Standard Deviation of the Implementation Shortfall: ${:,.2f}'.format(
        shortfall_hist.std()))
